...
const char* follow_link(char name[])
{
    ...
    count = snprintf(symLinkFollow, 18, "%s", filetype[SYML]);
    ...
    count = readlink(name, symLinkFollow + 18, 81);
...
#define permPrint(num,numHold,from) {if ((snprintf(numHold, 9, "%o", from)) == -1) {perror("something went wrong in printing?"); exit(EXIT_FAILURE);} \
                                if ((snprintf(num,4,"%s", numHold + 3)) == -1){perror("something went wrong in printing, the second?"); exit(EXIT_FAILURE);}}
...
void printStat(char name[], struct stat * file)
{
    ...
    timeaccess = localtime(&file->st_atim.tv_sec);
    timemod = localtime(&file->st_mtim.tv_sec);
    timestatus = localtime(&file->st_ctim.tv_sec);
...
//#define frees 
    ...
    timeaccess = gmtime(&file->st_atim.tv_sec);
    timemod = gmtime(&file->st_mtim.tv_sec);
    timestatus = gmtime(&file->st_ctim.tv_sec);

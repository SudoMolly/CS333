...
bool BIG_BOY = false;
int FULLSIZE = 0;
int FULLSIZESPLITS = 0;
const int MAXSPLIT = 10000;
...
void* deallocateOPTIONS(OPTIONS* data)
{
    ...
    inputExist = data->buff != NULL ? true : false;

    if (cExist) free(data->c_opt);
    if (xExist) free(data->x_opt);
    if (inputExist) free(data->buff);
    //data->input = NULL;
    data->buff = data->c_opt = data->x_opt = NULL;
    free(data);
    return NULL;
}
...
{
    if (prev == NULL) return (prev = charToCharStr(k));
    else prev[0] = k;
    return prev;
}
...
char* getInput(char* buffer)
{
    ...
    SHOW("into loop");
    while ((curr = read(STDIN_FILENO, hunHold, LIMIT)) != 0)
    {
        total_count  += strlen(hunHold);
        SHOW("LOOP");
        SHOWVAR("CURRENT HOLD", message = strToCharStr(hunHold[0], message));
        SHOWVAR("TOTAL", message = intToStr(total_count, message));
        if (buffer == NULL)
        {
        ...
        else 
        {
            ...
            SHOW("BUFF SIZE");
            SHOW(message = intToStr(strlen(buffer), message));
        }
        free(hunHold);
        hunHold = (char*) calloc(LIMIT + 1, sizeof(char));
    }
    if (strlen(hunHold) != 0)
    {
        ...
        else
        {
            ...
            buffer = hold;
            #ifdef DEBUG
                if (strcmp(hold, buffer) == 0)  SHOW("BUFFER TO HOLD");
                else SHOW("BUFFER NOT EQUAL HOLD");
                SHOW("BUFFER SIZE");
                SHOW(message = intToStr(strlen(buffer), message));
            #endif
            hold = NULL;
        ...
        hold = (char*) calloc (total_count + 1, sizeof(char));
        hold = strcpy(hold, buffer);
        buffer = strcat(buffer, hunHold);
...
void deduceSplits(void)
{
    int splitI;
    int size;
    if (!BIG_BOY) exit(EXIT_FAILURE);
    //ret = (BIGDATA*) malloc(sizeof(BIGDATA));
    splitI = 0;
    size = FULLSIZE;
    if (size > MAXSPLIT)
    {
        splitI = size / MAXSPLIT;
        if ((size - splitI * MAXSPLIT) > 0)
            ++splitI;
    }
    FULLSIZESPLITS = splitI;
    //ret->countLength = (int*) malloc(sizeof(int) * splitI);
    //ret->hold = (char**) malloc(sizeof(char*) * splitI);
}
...
int main(int argc, char** argv)
{
    ...
    do 
    {
        if (options->buff == NULL) SHOW("BUFF NULL");
        else SHOW("BUFF NOT NULL");
        options->buff = getInput(options->buff);
        SHOW("GOT BUFF");
        SHOWVAR("FIRST CHAR", message = strToCharStr(options->buff[0], message));
        SHOWVAR("LAST CHAR", message = strToCharStr(options->buff[strlen(options->buff) - 2], message));
        SHOWVAR("SIZE OF BUFF", message = intToStr(strlen(options->buff), message));
        SHOWVAR("BIG BOY?", (BIG_BOY ? "Yes" : "No"));
        if (options->buff == NULL) SHOW("\n\n\n===BUFFER END===\n\n\n");
        else if (options->buff != NULL)
        {
            if (BIG_BOY)
            {
                SHOW("IS BIG BOY");
                deduceSplits();
                threads = CREATE_THREAD_LIST((FULLSIZESPLITS));
                //forks = CREATE_FORK_LIST(FULLSIZESPLITS);
                DATA = (CAEXORDATA*) malloc(sizeof(CAEXORDATA) * FULLSIZESPLITS);
                options->OUT = (char*) calloc(FULLSIZE + 1, sizeof(char));
                if (options->OUT[FULLSIZE] == '\0' && options->OUT[0] == '\0')
                    fprintf(stderr, "out = %d", FULLSIZE + 1);
                options = bigSplitsEn(DATA, threads, options);
                fprintf(stdout, "\n\n\n\n--OUT--\n%s", options->buff);
                if (options->expected == Encrypt)
                {

                }
                else if (options->expected == Decrypt)
                {
            ...
            {
                if (options->d)
                    exitFail("Somehow decrypt option picked w/ encrypt chosen instead of decrypt", 2, options, options->buff);
                ...
                retText = option_e(options->c_opt, options->x_opt, options->buff);
                #ifdef DEBUG
                    SHOW("ENCRYPT CHOSEN");
                    VARSHOW("INPUT", options->buff);
            ...
            {
                if (options->e)
                    exitFail("Somehow encrypt option picked w/ decrypt chosen instead of encrypt", 2, options, options->buff);
                if (!options->d)
                    exitFail("Decrypt option chosen without decrypt option set to true", 2, options, options->buff);
                retText = option_d(options->c_opt, options->x_opt,options->buff);

...
void test_chars() {
  ...
  printf("char %c\nchar* %s\nchar*[] %s\t%s\t%s", c, c_ptr, c_ptr_arr[0],
         c_ptr_arr[1], c_ptr_arr[2]);
}

void test_ptr(int *ptr) { *ptr = 2; }
...
int test_length_num(void) {
  ...
  int curr_d = 1;
  int curr_e = 1;
  int nums[10] = {1,      10,      100,      1000,      10000,
...
void CS333_TEST_OUT(void) {
  ...
  printf("%*.*lf sec%s", 6, 2, ((double)(rand() % 1000000) / 10),
         print_spaces(14));
...
int verbose_add(thread_data *given) {
  ...
  snprintf(given->verbose, VERBOSE_LENGTH,
           "thread:%3d %6.2lf sec              DES: %5d               NT: %5d  "
           "            MD5: %5d           SHA256: %5d           SHA512: %5d   "
           "      YESCRYPT: %5d    GOST_YESCRYPT: %5d           BCRYPT: %5d  "
           "total: %8d  failed: %8d\n",
           thread, time, des, nt, md5, sha256, sha512, yes, gyes, b, total,
           failed);
  return total;
}
...
int test_cae() {
  ...
  for (int i = 0; i < strlen(string); ++i) {
    ...
    printf("%s\n||||||||||||||||||||||||||\n%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%"
           "c%c%c%c%c%c%c%c%c\t(%d)\n||||||||||||||||||||||||||\n",
           string, string_key[i], string_key[i], string_key[i], string_key[i],
           string_key[i], string_key[i], string_key[i], string_key[i],
           string_key[i], string_key[i], string_key[i], string_key[i],
           string_key[i], string_key[i], string_key[i], string_key[i],
           string_key[i], string_key[i], string_key[i], string_key[i],
           string_key[i], string_key[i], string_key[i], string_key[i],
           string_key[i], string_key[i], string_key[i] - 32);
...
int bitmasking(void) {
  ...
  for (int i = 0; i < 6; ++i) {
    printf("\nCharacter 1: %c  |  Character 2: %c\n--->%c", c_array[i],
           c_array[(i + 1) % 6], c_array[i] | c_array[(i + 1) % 6]);
  }
  printf("\nNumber modification:\n100 | 010 | 001 : %d\n100 | 200 | 400 : %d\n",
         (i_array[0] | i_array[1] | i_array[2]),
         (i_array[3] | i_array[0] | i_array[4]));
  return 0;
}
...
in_addr_t print_known_socket_type(char *addr) {
  in_addr_t ret = inet_addr(addr);
  in_addr_t ret2;
  inet_pton(AF_INET, addr, &ret2);
  printf("(inet_addr) from %s  -> %u\n", addr, ret);
  printf("(inet_pton) from %s  -> %u\n", addr, ret2);
  return ret;
}

void print_some_network_info(void) {
  printf("(Default) Any Inaddr -> 0x%X\n", INADDR_ANY);
  printf("(host to long) Any Inaddr -> 0x%X  (%u)", htonl(INADDR_ANY),
         (INADDR_ANY));
}

...
#define OPTIONS "i:o:vh"
    ...
    #define TRUE 1
#endif //TRUE
#define BUF_SIZE 50000 //#of bytes
#define DELIMITERS " ()\n" //BREAKUP DATA LINES
...
int main(int argc, char* argv[])
{
        ...
        while ((opt = getopt(argc,argv,OPTIONS)) != -1)
        {
            switch (opt)
            {
                ...
                case 'v':
                    ...
                    break;
                case 'h':
                    printf("sure, a lot of helpful information goes here\n");
                    exit(EXIT_SUCCESS);
                    //break;
                default:
                    break;
    ...
    NOISY_DEBUG_PRINT;
    if (is_verbose)
        printf("sorting %zu size structure vector with %d elements", sizeof(point_t),num_elements);
    fprintf(ofile,"---\n");
    output_data(ofile,vector, num_elements);
    NOISY_DEBUG_PRINT;

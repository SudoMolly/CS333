//update in place in binary file
//
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char* argv[])
{
    ...
    off_t offset = 0;
    off_t r_offset = 0;
    //off_t can be negative (so as to indicate direction) but sizeof is
    //unsigned (no negative bytes!)
    off_t size = sizeof(bin_file_t);
    {
        ...
        while ((opt = getopt(argc, argv, OPTIONS)) != -1)
        {
            ...
            {
                case 'i':
                    //sscanf returns num of matches in string, checking for
                    //match
    ...
    if (ofd < 0)
    {
        fprintf(stderr, "cannot open " FILE_NAME " for output");
        exit(EXIT_FAILURE);
    }

    r_offset = offset = record * size;
    //lseek returns # of bytes sought, check
